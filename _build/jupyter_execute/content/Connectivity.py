...
motor = data.extract_roi(mask_x[48])
motor = zscore(pd.DataFrame(motor, columns=['motor']))

# Build Design Matrix
csf_mask = Brain_Data(os.path.join('/Users/lukechang/Dropbox/TV_fMRI/', 'Masks', 'csf.nii.gz'))
csf_mask = csf_mask.threshold(upper=.7, binarize=True)
csf = zscore(pd.DataFrame(data.extract_roi(csf_mask), columns=['csf']))

spikes = smoothed.find_spikes(global_spike_cutoff=3, diff_spike_cutoff=3)
covariates = pd.read_csv(layout.get(subject=sub, scope='derivatives', extension='.tsv')[0].path, sep='\t')
mc = covariates[['trans_x','trans_y','trans_z','rot_x', 'rot_y', 'rot_z']]
mc_cov = make_motion_covariates(mc, tr)
dm = Design_Matrix(pd.concat([motor, csf, mc_cov, spikes.drop(labels='TR', axis=1)], axis=1), sampling_freq=1/tr)
dm = dm.add_poly(order=2, include_lower=True)
...
# Write out Motor Connectivity Map
stats['beta'][0].write(f'{sub}_motor_connectivity_map.nii.gz')
...
[x for x in layout.get_subjects() if x != 'S15']
...
sub_list = [x for x in layout.get_subjects() if x != 'S15']

for sub in tqdm(sub_list):
    # Print which subject we are on
    print(sub)
...
file_list = glob.glob('*_motor_connectivity_map.nii.gz')
file_list.sort()
all_betas = Brain_Data(file_list)

all_betas

level_two_stats = all_betas.ttest(threshold_dict={'fdr':.005})
level_two_stats['thr_t'].iplot()
