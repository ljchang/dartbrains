...
def _(np, plt):
    ...
    data = np.random.multivariate_normal(_mu, _cov, size=n)
...
@app.cell
def _(data_1, np, plt):
    plt.bar(['x1', 'x2'], np.mean(data_1, axis=0))
    return
...
def _(np, plt):
    ...
    r2 = 0.5
    r3 = 0.9
    _mu = np.array([5.0, 0.0, 10.0])
    _cov = np.array([[1, r1, r2], [r1, 1, r3], [r2, r3, 1]])
    data_2 = np.random.multivariate_normal(_mu, _cov, size=n_1)
    _ax = plt.axes(projection='3d')
    _ax.scatter3D(data_2[:, 0], data_2[:, 1], data_2[:, 2], alpha=0.4)
    return
...
def _(np, plt):
    ...
    _mu = np.array([0, 0])
    _cov = np.array([[1, _r], [_r, 1]])
    data_3 = np.random.multivariate_normal(_mu, _cov, size=n_2)
    plt.scatter(data_3[:, 0], data_3[:, 1], color='r', marker='o', s=30, alpha=0.25)
    plt.xlabel('Variable 1', fontsize=16)
    plt.ylabel('Variable 2', fontsize=16)
    plt.title('Scatterplot', fontsize=18)
    plt.annotate(f'r={_r}', xy=(-2, 2), xycoords='data', fontsize=14, color='red')
    return
...
def _(np, plt):
    ...
    _r = 0.1
    sim1 = simulate_xy(n=500, r=_r)
    _ax[0, 0].scatter(sim1[:, 0], sim1[:, 1], color='r', marker='o', s=30, alpha=0.25)
    _ax[0, 0].set_title(f'r={_r}', fontsize=16)
    _ax[0, 0].set_ylabel('Y', fontsize=16)
    _r = 0.3
    sim1 = simulate_xy(n=500, r=_r)
    _ax[0, 1].scatter(sim1[:, 0], sim1[:, 1], color='r', marker='o', s=30, alpha=0.25)
    _ax[0, 1].set_title(f'r={_r}', fontsize=16)
    _r = 0.6
    sim1 = simulate_xy(n=500, r=_r)
    _ax[1, 0].scatter(sim1[:, 0], sim1[:, 1], color='r', marker='o', s=30, alpha=0.25)
    _ax[1, 0].set_title(f'r={_r}', fontsize=16)
    _ax[1, 0].set_xlabel('X', fontsize=16)
    _ax[1, 0].set_ylabel('Y', fontsize=16)
    _r = 0.9
    sim1 = simulate_xy(n=500, r=_r)
    _ax[1, 1].scatter(sim1[:, 0], sim1[:, 1], color='r', marker='o', s=30, alpha=0.25)
...
@app.cell
def _(plt, simulate_xy_1, sns):
    sns.regplot(data=simulate_xy_1(), x='Variable1', y='Variable2')
    plt.title('Scatterplot', fontsize=18)
    return
...
@app.cell
def _(plt, simulate_xy_1, sns):
    sns.jointplot(data=simulate_xy_1(), x='Variable1', y='Variable2')
...
@app.cell
def _(pd, simulate_xy_1, sns):
    data_4 = pd.concat([simulate_xy_1(r=0.9), simulate_xy_1(r=0.3)], axis=1)
    data_4.columns = ['V1', 'V2', 'V3', 'V4']
    sns.pairplot(data=data_4)
    return
...
@app.cell
def _(pd, plt, simulate_xy_1, sns):
    data1 = simulate_xy_1(r=0.9)
    data1['Group'] = 'Group1'
    data1['Variable1'] = data1['Variable1'] + 1
    data2 = simulate_xy_1(r=0.3) + 2
    data2['Group'] = 'Group2'
    data_5 = pd.concat([data1, data2], axis=0)
    data_long = data_5.melt(id_vars='Group', value_vars=['Variable1', 'Variable2'], var_name='Variable', value_name='Data')
...
@app.cell
def _(pd, simulate_xy_1, sns):
    data_6 = pd.concat([simulate_xy_1(r=0.9), simulate_xy_1(r=0.3)], axis=1)
    data_6.columns = ['V1', 'V2', 'V3', 'V4']
    corr = data_6.corr()
...
def _(pd, plt, sns):
    ...
    _a[0].set_ylabel('Salary', fontsize=18)
    sns.scatterplot(data=data_8, x='YearsSinceHighestDegree', y='Salary', hue='Sex', ax=_a[1])
    _a[1].set_ylabel('Salary', fontsize=18)
