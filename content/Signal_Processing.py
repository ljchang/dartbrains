...
@app.cell
def _(a, np, plt):
    b_1 = a + np.random.randn(20)
    plt.scatter(a, b_1)
    plt.ylabel('B', fontsize=18)
    plt.xlabel('A', fontsize=18)
    plt.title('Scatterplot', fontsize=18)
    print(f'Dot Product: {np.dot(a, b_1)}')
    return
...
def _(np, plt):
    ...
    kernel = np.zeros(10)
    kernel[2:8] = 1
    _f, a_1 = plt.subplots(ncols=2, figsize=(20, 5))
    a_1[0].plot(signal, linewidth=2)
    a_1[0].set_xlabel('Time', fontsize=18)
    a_1[0].set_ylabel('Signal Intensity', fontsize=18)
    a_1[0].set_title('Signal ', fontsize=18)
    a_1[1].plot(kernel, linewidth=2, color='red')
    a_1[1].set_xlabel('Time', fontsize=18)
    a_1[1].set_ylabel('Intensity', fontsize=18)
    a_1[1].set_title('Kernel ', fontsize=18)
    return kernel, n_samples, signal
...
@app.cell
def _(kernel, n_samples, np, plt):
    shifted_kernel = np.zeros((n_samples, n_samples + len(kernel) - 1))
    for _i in range(n_samples):
        shifted_kernel[_i, _i:_i + len(kernel)] = kernel
...
@app.cell
def _(np, plt, shifted_kernel, signal):
...
@app.cell
def _(convolved_signal, kernel, signal):
...
@app.cell
def _(kernel, n_samples, np, plt):
    signal_1 = np.zeros(n_samples)
    signal_1[np.random.randint(0, n_samples, 5)] = np.random.randint(1, 5, 5)
    _f, a_2 = plt.subplots(nrows=2, figsize=(18, 6), sharex=True)
    a_2[0].plot(signal_1, linewidth=2)
    a_2[0].set_ylabel('Intensity', fontsize=18)
    a_2[0].set_title('Timeseries of spikes with varying intensities', fontsize=18)
    a_2[1].plot(np.convolve(signal_1, kernel), linewidth=2)
    a_2[1].set_ylabel('Intensity', fontsize=18)
    a_2[1].set_title('Signal convolved with boxcar kernel', fontsize=18)
    a_2[1].set_xlabel('Time', fontsize=18)
    return
...
@app.cell
def _(glover_hrf, plt, tr):
    hrf_1 = glover_hrf(tr, oversampling=1)
    plt.plot(hrf_1, linewidth=2, color='red')
    plt.ylabel('Intensity', fontsize=18)
    plt.xlabel('Time', fontsize=18)
    plt.title('Hemodynamic Response Function', fontsize=18)
    return (hrf_1,)
...
@app.cell
def _(hrf_1, n_samples, np, plt):
    signal_2 = np.zeros(n_samples)
    signal_2[np.random.randint(0, n_samples, 5)] = np.random.randint(1, 5, 5)
    _f, a_3 = plt.subplots(nrows=2, figsize=(18, 6), sharex=True)
    a_3[0].plot(signal_2, linewidth=2)
    a_3[1].plot(np.convolve(signal_2, hrf_1), linewidth=2)
    a_3[0].set_ylabel('Intensity', fontsize=18)
    a_3[0].set_title('Timeseries of spikes with varying intensities', fontsize=18)
    a_3[1].set_ylabel('Intensity', fontsize=18)
    a_3[1].set_xlabel('Time', fontsize=18)
    a_3[1].set_title('Signal convolved with HRF kernel', fontsize=18)
    return
...
def _(plt):
    ...
    _freq = 5
    theta = 0
    simulation = _amplitude * sin(2 * pi * _freq * time + theta)
    plt.figure(figsize=(12, 5))
    plt.plot(time, simulation, linewidth=2)
    plt.title('Sine Wave', fontsize=18)
    plt.xlabel('Time', fontsize=18)
    plt.ylabel('Amplitude', fontsize=18)
    return arange, pi, sin, theta
...
def _(arange, pi, plt, sin):
    ...
    def plot_oscillation(amplitude=5, frequency=5, theta=1):
        ...
        simulation = _amplitude * sin(2 * pi * frequency * time + theta)
...
@app.cell
def _(arange, np, pi, plt, sin):
    sampling_freq_1 = 500
    _freq = [3, 10, 5, 15, 35]
    _amplitude = [5, 15, 10, 5, 7]
    _phases = pi * np.array([1 / 7, 1 / 8, 1, 1 / 2, -1 / 4])
    time_1 = arange(-1, 1 + 1 / sampling_freq_1, 1 / sampling_freq_1)
    sine_waves = []
    for _i, _f in enumerate(_freq):
        sine_waves.append(_amplitude[_i] * sin(2 * pi * _f * time_1 + _phases[_i]))
    sine_waves = np.array(sine_waves)
    _f, a_4 = plt.subplots(nrows=5, ncols=1, figsize=(12, 5), sharex=True)
    for _i, _x in enumerate(_freq):
        a_4[_i].plot(sine_waves[_i, :], linewidth=2)
    a_4[0].set_title('Sine waves oscillating at different frequencies', fontsize=18)
    a_4[_i].set_xlabel('Time', fontsize=18)
    plt.tight_layout()
    return (sine_waves,)
...
def _(arange, np, pi, plt, sin):
    ...
    _amplitude = [5, 15, 10, 5, 7]
    _phases = pi * np.array([1 / 7, 1 / 8, 1, 1 / 2, -1 / 4])
    time_2 = arange(-1, 1 + 1 / sampling_freq_2, 1 / sampling_freq_2)
    sine_waves_1 = []
    for _i, _f in enumerate(_freq):
        sine_waves_1.append(_amplitude[_i] * sin(2 * pi * _f * time_2 + _phases[_i]))
    sine_waves_1 = np.array(sine_waves_1)
    _f, a_5 = plt.subplots(nrows=5, ncols=1, figsize=(12, 5), sharex=True)
    for _i, _x in enumerate(_freq):
        a_5[_i].plot(sine_waves_1[_i, :], linewidth=2)
    a_5[0].set_title('Sine waves oscillating at different frequencies', fontsize=18)
    a_5[_i].set_xlabel('Time', fontsize=18)
    plt.tight_layout()
    plt.figure(figsize=(12, 3))
    plt.plot(np.sum(sine_waves_1, axis=0), linewidth=2)
    plt.title('Sum of all of the sine waves', fontsize=18)
    plt.xlabel('Time', fontsize=18)
    plt.tight_layout()
    return
...
def _(arange, np, pi, plt, sin):
    ...
    _amplitude = [5, 15, 10, 5, 7]
    _phases = pi * np.array([1 / 7, 1 / 8, 1, 1 / 2, -1 / 4])
    time_3 = arange(-1, 1 + 1 / sampling_freq_3, 1 / sampling_freq_3)
    sine_waves_2 = []
    for _i, _f in enumerate(_freq):
        sine_waves_2.append(_amplitude[_i] * sin(2 * pi * _f * time_3 + _phases[_i]))
    sine_waves_2 = np.array(sine_waves_2)
    noise = 5 * np.random.randn(sine_waves_2.shape[1])
    signal_3 = np.sum(sine_waves_2, axis=0) + noise
    plt.figure(figsize=(12, 3))
    plt.plot(signal_3, linewidth=2)
    plt.title('Sum of sine waves plus white noise', fontsize=18)
    plt.ylabel('Intensity', fontsize=18)
    plt.xlabel('Time', fontsize=18)
    return sampling_freq_3, signal_3, time_3
...
@app.cell
def _(imag, np, pi, plt, real, sampling_freq_3, theta, time_3):
    from mpl_toolkits import mplot3d
    frequency = 5
    z = np.exp(1j * (2 * pi * frequency * time_3 + theta))
    fig = plt.figure(figsize=(15, 10))
    ax = fig.add_subplot(2, 2, 1, projection='3d')
    ax.plot(np.arange(0, len(time_3)) / sampling_freq_3, real(z), imag(z))
    ...
    ax.plot(np.arange(0, len(time_3)) / sampling_freq_3, real(z))
    ax.set_xlabel('Time (sec)', fontsize=16)
    ax.set_ylabel('Real(z)', fontsize=16)
    ax.set_title('Projecting on Real and Time', fontsize=18)
    ax = fig.add_subplot(2, 2, 4)
    ax.plot(np.arange(0, len(time_3)) / sampling_freq_3, imag(z))
    ax.set_xlabel('Time (sec)', fontsize=16)
    ax.set_ylabel('Imaginary(z)', fontsize=16)
...
@app.cell
def _(np, pi, plt, signal_3):
    from numpy import exp
    time_4 = np.arange(0, len(signal_3), 1) / len(signal_3)
    sine_waves_3 = []
    for _i in range(len(signal_3)):
        sine_waves_3.append(exp(-1j * 2 * pi * _i * time_4))
    sine_waves_3 = np.array(sine_waves_3)
    _f, a_6 = plt.subplots(nrows=5, figsize=(12, 8), sharex=True)
    for _i in range(0, 5):
        a_6[_i].plot(sine_waves_3[_i, :], linewidth=2)
    a_6[0].set_title('Bank of sine waves', fontsize=18)
    a_6[_i].set_xlabel('Time', fontsize=18)
    plt.tight_layout()
    return (sine_waves_3,)
...
@app.cell
def _(np, plt, sine_waves_3):
    plt.figure(figsize=(12, 12))
    plt.imshow(np.real(sine_waves_3))
...
@app.cell
def _(np, signal_3, sine_waves_3):
    fourier = 2 * np.dot(signal_3, sine_waves_3) / len(signal_3)
    return (fourier,)
...
@app.cell
def _(fourier, np, plt, sampling_freq_3, signal_3):
    from numpy.fft import fftfreq
    _freq = fftfreq(len(signal_3), 1 / sampling_freq_3)
    plt.figure(figsize=(12, 5))
    plt.plot(_freq[:80], np.abs(fourier)[0:80], linewidth=2)
...
@app.cell
def _(fourier, np, plt, sine_waves_3):
    plt.figure(figsize=(12, 5))
    plt.plot(np.dot(fourier, sine_waves_3) / 2)
    plt.ylabel('Intensity', fontsize=18)
    plt.xlabel('Time', fontsize=18)
...
@app.cell
def _(np, plt, signal_3):
    from numpy.fft import fft, ifft
    fourier_fft = fft(signal_3)
    plt.figure(figsize=(12, 5))
    plt.plot(np.arange(0, 80) / 2, 2 * np.abs(fourier_fft[0:80]) / len(signal_3), linewidth=2)
    plt.ylabel('Amplitude', fontsize=18)
    plt.xlabel('Frequency (Hz)', fontsize=18)
    plt.title('Frequency domain representation of signal derived from fast fourier transform', fontsize=18)
    return fourier_fft, ifft
...
def _(np, plt):
    ...
    frequency_cutoff = 25
    sampling_frequency = 500
    b_2, a_7 = butter(_filter_order, frequency_cutoff, btype='high', output='ba', fs=sampling_frequency)

    def rad_sample_to_hz(x, fs):
        return _x * fs / (2 * np.pi)
    ...
    plot_filter(b_2, a_7, sampling_frequency)
    return butter, filtfilt, plot_filter
...
@app.cell
def _(butter, plot_filter):
    _filter_order = 2
    frequency_cutoff_1 = 25
    sampling_frequency_1 = 500
    b_3, a_8 = butter(_filter_order, frequency_cutoff_1, btype='high', output='ba', fs=sampling_frequency_1)
    plot_filter(b_3, a_8, sampling_frequency_1)
    return a_8, b_3, frequency_cutoff_1, sampling_frequency_1
...
@app.cell
def _(butter, frequency_cutoff_1, ifft, plt, sampling_frequency_1):
    from scipy.signal import sosfreqz
    _filter_order = 8
    sos = butter(_filter_order, frequency_cutoff_1, btype='high', output='sos', fs=sampling_frequency_1)
...
@app.cell
def _(a_8, b_3, filtfilt, plt, signal_3):
    _filtered = filtfilt(b_3, a_8, signal_3)
    plt.figure(figsize=(20, 5))
    plt.plot(signal_3, linewidth=2)
    plt.plot(_filtered, linewidth=2)
    plt.ylabel('Intensity', fontsize=18)
    plt.xlabel('Time', fontsize=18)
...
@app.cell
def _(butter, filtfilt, plt, signal_3):
    _filter_order = 2
    frequency_cutoff_2 = 10
    sampling_frequency_2 = 500
    b_4, a_9 = butter(_filter_order, frequency_cutoff_2, btype='low', output='ba', fs=sampling_frequency_2)
    _filtered = filtfilt(b_4, a_9, signal_3)
    plt.figure(figsize=(20, 5))
    plt.plot(signal_3, linewidth=2)
    plt.plot(_filtered, linewidth=4)
    plt.ylabel('Intensity', fontsize=18)
    plt.xlabel('Time', fontsize=18)
    plt.legend(['Original', 'Filtered'], fontsize=18)
    return
...
def _(butter, plot_filter):
    _filter_order = 10
    frequency_cutoff_3 = 10
    sampling_frequency_3 = 500
    b_5, a_10 = butter(_filter_order, frequency_cutoff_3, btype='low', output='ba', fs=sampling_frequency_3)
    plot_filter(b_5, a_10, sampling_frequency_3)
...
@app.cell
def _(butter, filtfilt, plt, sampling_frequency_3, signal_3):
    _filter_order = 2
    _lowcut = 7
    _highcut = 13
    b_6, a_11 = butter(_filter_order, [_lowcut, _highcut], btype='bandpass', output='ba', fs=sampling_frequency_3)
    _filtered = filtfilt(b_6, a_11, signal_3)
    plt.figure(figsize=(20, 5))
    plt.plot(signal_3, linewidth=2)
    plt.plot(_filtered, linewidth=4)
    plt.ylabel('Intensity', fontsize=18)
...
@app.cell
def _(butter, filtfilt, plot_filter, plt, sampling_frequency_3, signal_3):
    _filter_order = 2
    _lowcut = 8
    _highcut = 12
    b_7, a_12 = butter(_filter_order, [_lowcut, _highcut], btype='bandstop', output='ba', fs=sampling_frequency_3)
    plot_filter(b_7, a_12, sampling_frequency_3)
    _filtered = filtfilt(b_7, a_12, signal_3)
    plt.figure(figsize=(20, 5))
    plt.plot(signal_3, linewidth=2)
    plt.plot(_filtered, linewidth=2)
    plt.ylabel('Intensity', fontsize=18)
